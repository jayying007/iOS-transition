...
- (instancetype)init {
    ...
    if (self) {
        weakify(self);
        [[NSNotificationCenter defaultCenter] addObserverForName:kFinishInteraction object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
            strongify(self);
            if (self == nil) {
                return;
            }
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:kGestureEnd object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
            [self hideFloatFan];
        }];
...
- (FloatFan *)floatFan {
    if (_floatFan == nil) {
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:kGestureChangePercent object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
...
- (void)handleTapFloatButton {
    ...
    DetailViewController *vc = [[DetailViewController alloc] initWithText:Service(FloatWindowMgr).text];
