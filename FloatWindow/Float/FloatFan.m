...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:kGestureChangeTouchPoint object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
            NSValue *value = note.userInfo[@"touchPoint"];
            CGPoint touchPoint = [value CGPointValue];
            CGFloat distance = sqrt(pow(WINDOW.width - touchPoint.x, 2) + pow(WINDOW.height - touchPoint.y, 2));
