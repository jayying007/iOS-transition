...
- (void)handleEdgePanGesture:(UIScreenEdgePanGestureRecognizer *)edgePanGesture {
    ...
    if (edgePanGesture.state == UIGestureRecognizerStateChanged) {
        ...
        [self notifyGestureChangeTouchPoint:touchPoint];
               ...
               edgePanGesture.state == UIGestureRecognizerStateCancelled ||
...
- (void)notifyGestureChangePercent:(CGFloat)percent {
    [[NSNotificationCenter defaultCenter] postNotificationName:kGestureChangePercent object:nil userInfo:@{@"percent" : [NSNumber numberWithDouble:percent]}];
}

- (void)notifyGestureChangeTouchPoint:(CGPoint)touchPoint {
    [[NSNotificationCenter defaultCenter] postNotificationName:kGestureChangeTouchPoint object:nil userInfo:@{@"touchPoint" : [NSValue valueWithCGPoint:touchPoint]}];
}
