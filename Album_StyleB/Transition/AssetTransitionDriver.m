...
- (instancetype)initWithOperation:(UINavigationControllerOperation)operation context:(id<UIViewControllerContextTransitioning>)transitionContext panGesture:(UIPanGestureRecognizer *)panGesture {
    self = [super init];
    if (self) {
        ...
        id<AssetTransitioning> fromAssetTransitioning = nil;
        if ([fromVC conformsToProtocol:@protocol(AssetTransitioning)]) {
            fromAssetTransitioning = (id<AssetTransitioning>)fromVC;
        }
        id<AssetTransitioning> toAssetTransitioning = nil;
        if ([toVC conformsToProtocol:@protocol(AssetTransitioning)]) {
            toAssetTransitioning = (id<AssetTransitioning>)toVC;
        }
        ...
        UIVisualEffect *targetEffect = (operation == UINavigationControllerOperationPop) ? nil : [UIBlurEffect effectWithStyle:UIBlurEffectStyleLight];
        ...
        self.transitionAnimator = [[UIViewPropertyAnimator alloc] initWithDuration:transitionDuration curve:UIViewAnimationCurveEaseOut animations:^{
...
- (AssetTransitionItem *)itemAtPoint:(CGPoint)point {
...
- (CGFloat)progressStep:(CGPoint)translation {
    return (self.operation == UINavigationControllerOperationPush ? -1.0 : 1.0) * translation.y / CGRectGetMidY(self.transitionContext.containerView.bounds);
}
...
- (void)updateItemsForInteractive:(CGPoint)translation {
    ...
    for (AssetTransitionItem *item in self.items) {
        ...
        CGFloat itemPercentComplete = MAX(-0.05, MIN(1.05, (currentSize.width - initialSize.width) / (finalSize.width - initialSize.width) + progressStep));
        ...
        imageView.center = CGPointMake(imageView.center.x + (translation.x + item.touchOffset.x - scaledOffset.x), imageView.center.y + (translation.y + item.touchOffset.y - scaledOffset.y));
...
- (UIViewAnimatingPosition)completionPosition {
    ...
    if (self.operation == UINavigationControllerOperationPush) {
        if (isFlickUp) return UIViewAnimatingPositionEnd;
        if (isFlickDown) return UIViewAnimatingPositionStart;
    } else if (self.operation == UINavigationControllerOperationPop) {
        if (isFlickDown) return UIViewAnimatingPositionEnd;
        if (isFlickUp) return UIViewAnimatingPositionStart;
...
- (CGVector)convertVelocity:(CGPoint)velocity forItem:(AssetTransitionItem *)item {
...
+ (UIViewPropertyAnimator *)propertyAnimator:(CGVector)initialVelocity {
    UISpringTimingParameters *timingParameters = [[UISpringTimingParameters alloc] initWithMass:4.5 stiffness:800 damping:95 initialVelocity:initialVelocity];
